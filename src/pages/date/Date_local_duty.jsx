  ...
  const onSubmit = async (data) => {
    try {
      for (const passenger of data.passengers) {
        ...
        passenger.trip_type = "Date Local Duty";
  ...
  return (
    <div className="min-h-full w-full bg-blue-50 sm:px-15 py-4 flex items-start justify-center hide-scrollbar">
      <form
        ...
        className="bg-white shadow-xl rounded-2xl w-full max-w-full px-4 space-y-1 border border-blue-200"
      >
        <h2 className="text-2xl font-bold mb-5 mt-5 text-blue-700 border-b pb-2  text-center">
          Date Local duty
        </h2>
        ...
        <div className="flex flex-col-2 sm:flex-row gap-4 items-center">
          <div className="w-1/2">
            ...
            <input
              ...
              className="border w-full rounded-lg px-3 py-2 text-gray-800"
            />
            {errors.from_date && (
          ...
          <div className="flex flex-col w-1/2">
            ...
            <select
              className="border rounded-lg px-3 w-full py-2"
              onChange={(e) => {
        ...
        {/* Table Header */}
        <div className={`grid max-sm:hidden ${fields.length > 1 ? "grid-cols-11" : "grid-cols-11"} gap-4 font-semibold text-gray-700 text-center bg-blue-100 p-2 rounded-t-lg mt-6`}>
          <div>Passenger</div>
          <div>From Time</div>
          <div>From</div>
          <div>To</div>
        ...
        {fields.map((field, index) => (
          ...
          <div
            key={field.id}
            className={`grid grid-cols-1 ${fields.length > 1 ? "sm:grid-cols-11" : "sm:grid-cols-11"
              } gap-4 items-center mt-5 bg-blue-50 py-6  px-4 rounded-lg border border-blue-700`}
          >
            {/* Passenger */}
            <select
            ...
            <input
              type="number"
              {...register(`passengers.${index}.local_package_km`)}
              className="border rounded-lg px-2 py-1 text-gray-800"
              placeholder="Km"
              onChange={(e) => {
                setValue(`passengers.${index}.local_package_km`, e.target.value);
                setBookingData((prev) => ({ ...prev, local_package_km: e.target.value }));
              }}
            />
            ...
            <input
              type="number"
              {...register(`passengers.${index}.local_package_hr`)}
              className="border rounded-lg px-2 py-1 text-gray-800"
              placeholder="Hr"
              onChange={(e) => {
                setValue(`passengers.${index}.local_package_hr`, e.target.value);
                setBookingData((prev) => ({ ...prev, local_package_hr: e.target.value }));
              }}
            />

            {/* Local Package Charge */}
            <input
              type="number"
              {...register(`passengers.${index}.local_package_charge`)}
              className="border rounded-lg px-2 py-1 text-gray-800"
              placeholder="Charge"
              onChange={(e) => {
                setValue(`passengers.${index}.local_package_charge`, e.target.value);
                setBookingData((prev) => ({ ...prev, local_package_charge: e.target.value }));
              }}
            />
            {/* Extra KM Charge */}
              ...
              <input
                type="number"
                {...register(`passengers.${index}.local_extra_km_charge`)}
              ...
              <input
                type="number"
                {...register(`passengers.${index}.local_extra_hr_charge`)}
                placeholder="₹ extra HR"
                min="0"
                className="mt-1 border rounded-lg px-3 py-2 w-full"
              />
